class Client < ActiveRecord::Base
  ...
  after_create  { |client| client.create_notification(:note_type => 'client_created') }
  after_update  { |client| client.create_notification(:note_type => 'client_updated') }
  after_destroy { |client| client.create_notification(:note_type => 'client_deleted') }
  ...
  def create_notification(*opts)
    options = {:portfolio_id => self.portfolio_id, :client_id => self.id, :item_type => self.class.to_s, :item_id => self.id}.merge( opts.extract_options! )
